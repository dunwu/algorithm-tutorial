...
/**
 * A tree can be defined recursively (locally) as a collection of nodes (starting at a
 * root node), where each node is a data structure consisting of a value, together with a
 * list of nodes (the "children"), with the constraints that no node is duplicated. A tree
 * can be defined abstractly as a whole (globally) as an ordered tree, with a value
