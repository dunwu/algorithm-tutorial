...
import io.github.dunwu.leetcode.common.ITree;
...
/**
 * B树是一种树数据结构，可以对数据进行排序，并允许以对数时间进行搜索，顺序访问，插入和删除。
 * <p>
...
public class BinaryTree<T extends Comparable<T>> implements ITree<T> {
    ...
    public java.util.Collection<T> toCollection() {
        return (new JavaCompatibleBinaryTree<T>(this));
    }
    ...
    private static class TreePrinter {
        ...
        public static <T extends Comparable<T>> String getString(BinaryTree<T> tree) {
            if (tree.root == null) {
    ...
    public static class JavaCompatibleBinaryTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {

        private BinaryTree<T> tree = null;

        public JavaCompatibleBinaryTree(BinaryTree<T> tree) {
            this.tree = tree;
        }
        ...
        public java.util.Iterator<T> iterator() {
            return (new BinaryTreeIterator<T>(this.tree));
        }
        ...
        private static class BinaryTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {

            private BinaryTree<C> tree = null;
            ...
            protected BinaryTreeIterator(BinaryTree<C> tree) {
                this.tree = tree;
            ...
            public boolean hasNext() {
                boolean toVisitSizeNotZero = toVisit.size() > 0;
                boolean lastNodeNotZero = lastNode != null && index < lastNode.keysSize;
                if (lastNodeNotZero || toVisitSizeNotZero) {
                    return true;
                }
                return false;
            }
