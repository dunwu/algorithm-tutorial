...
public class JavaMapTest {
	...
	public static <K, V, T extends Comparable<T>> boolean testJavaMap(java.util.Map<K, V> map, Class<T> type,
			String name, Integer[] unsorted, Integer[] sorted, Integer _invalid) {
		...
		addInOrderAndRemoveInOrder(map, type, name, unsorted, _invalid);
		addInReverseOrderAndRemoveInReverseOrder(map, type, name, unsorted, _invalid);
		addInOrderAndRemoveInReverseOrder(map, type, name, unsorted, _invalid);

		addInOrderAndRemoveInOrder(map, type, name, sorted, _invalid);
		addInReverseOrderAndRemoveInReverseOrder(map, type, name, sorted, _invalid);
		addInOrderAndRemoveInReverseOrder(map, type, name, sorted, _invalid);
	...
	private static <K, V, T extends Comparable<T>> boolean addInOrderAndRemoveInOrder(java.util.Map<K, V> map,
			Class<T> keyType, String name, Integer[] data, Integer _invalid) {
		...
		if (!testMapEntrySet(map, keyType, data))
			return false;
	...
	private static <K, V, T extends Comparable<T>> boolean addInReverseOrderAndRemoveInReverseOrder(
			java.util.Map<K, V> map, Class<T> keyType, String name, Integer[] data, Integer _invalid) {
	...
	private static <K, V, T extends Comparable<T>> boolean addInOrderAndRemoveInReverseOrder(java.util.Map<K, V> map,
			Class<T> keyType, String name, Integer[] data, Integer _invalid) {
		for (int i = 0; i < data.length; i++) {
			Integer item = data[i];
			K k = null;
