...
public class JavaCollectionTest {
    ...
    private static <T extends Comparable<T>> boolean addAndRemoveInOrder(Collection<T> collection, Class<T> type,
        String name, Integer[] data, Integer _invalid) {
        ...
        if (collection instanceof java.util.List && (!ListIteratorTest
            .testListIterator(((java.util.List<T>) collection).listIterator(), type, data, data.length))) {
    ...
    public static <T extends Comparable<T>> boolean addInOrderRemoveInReverseOrder(Collection<T> collection,
        Class<T> type, String name, Integer[] data, Integer _invalid) {
        T invalid = Utils.parseT(_invalid, type);

        // Add in order (from index zero to length) and then remove in reverse (from index
        // length-1 to zero) order
        for (int i = 0; i < data.length; i++) {
            Integer value = data[i];
            T item = Utils.parseT(value, type);
            boolean added = collection.add(item);
    ...
    private static <T extends Comparable<T>> boolean addInReverseOrderAndRemoveInOrder(Collection<T> collection,
        Class<T> type, String name, Integer[] data, Integer _invalid) {
        T invalid = Utils.parseT(_invalid, type);

        // Add in reverse (from index length-1 to zero) order and then remove in order
        // (from index zero to length)
        for (int i = data.length - 1; i >= 0; i--) {
            Integer value = data[i];
            T item = Utils.parseT(value, type);
            boolean added = collection.add(item);
